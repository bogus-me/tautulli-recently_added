...
# ═════ Bilder & Links ════════════════════════════════════════
def fallback_thumb(item):
    url = (item.get("grandparent_thumb") or item.get("thumb") or
           item.get("parent_thumb")      or item.get("art")   or
           item.get("poster")            or "")
    if url.startswith("/"):
        url = f"{TAUTULLI_URL.rstrip('/')}{url}"
    return url or None
...
def get_tmdb_backdrop(tmdb_id: str, is_movie: bool, fb_item: dict) -> Optional[str]:
    mtype = "movie" if is_movie else "tv"
    try:
    ...
    return fallback_thumb(fb_item)

def get_tmdb_poster(tmdb_id: str, is_movie: bool, fb_item: dict) -> Optional[str]:
    mtype = "movie" if is_movie else "tv"
    try:
        r = tget(f"https://api.themoviedb.org/3/{mtype}/{tmdb_id}/images",
                 params={"api_key": TMDB_API_KEY, "include_image_language": "de,null,en"},
...
def build_title(item: Dict) -> str:
    ...
    if len(clean) <= SINGLE_LINE_LIMIT:
        return clean
    m = re.search(r"\s[-–:|]\s", clean)
    if m:
        first, second = clean[:m.start()].strip(), clean[m.end():].strip()
    else:
        mid = clean.rfind(" ", 0, len(clean) // 2)
        first, second = clean[:mid].strip(), clean[mid:].strip()
    if len(second) > MAX_LINE_LEN:
        second = second[:MAX_LINE_LEN].rstrip() + "…"
    return f"{first}\n{INDENT}{second}"
...
def build_embed(item: dict, season_meta: dict = {}, series_meta: dict = {}) -> Dict:
    ...
    if plot:
        ...
        if len(norm) > PLOT_LIMIT: norm = norm[:PLOT_LIMIT].rstrip() + " …"
        plot_txt = insert_line_breaks(norm)
        if style in {"boxed", "telegram"}: plot_txt = indent_block(plot_txt)
    else:
        plot_txt = "_Leider liegen zu diesem Titel noch keine weiteren Informationen vor._"
    h_title = f"📝 Handlung – Starring ▸ {actor}" if mtype in {"season", "show", "episode"} and actor else "📝 Handlung"
    ...
    main_info = "Autor: " + writers[0]       if writers   else \
                "Produzent: " + producers[0] if producers else \
                "Regie: " + directors[0]     if directors else ""
    ...
    # ----- Bild ------------------------------------------------
    def ph_url(w: int, h: int) -> str:
        return f"https://via.placeholder.com/{w}x{h}.png?text=Kein+Bild"

    img_ok = False
    if tmdb_id:
        if style == "telegram":
            poster = get_tmdb_poster(tmdb_id, mtype == "movie", item)
            if poster:
                embed["image"] = {"url": poster}
                img_ok = True
            else:
                embed["image"] = {"url": ph_url(500, 750)}
        else:
            back = get_tmdb_backdrop(tmdb_id, mtype == "movie", item)
            if back:
                embed["image"] = {"url": back}
                img_ok = True
            else:
                embed["image"] = {"url": ph_url(780, 439)}
    else:
        embed["image"] = {"url": ph_url(500, 750) if style == "telegram" else ph_url(780, 439)}
